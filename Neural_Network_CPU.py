...
class NeuralNetwork:
    ...
    def generate_weights(self):
        ...
        self.weights_mat_list.append(
            np.random.randn(self.input_size, self.neurons_in_layers[0]) * np.sqrt(2 / self.input_size))

        # hidden layers
        for i in range(self.hidden_layer_num - 1):
            self.weights_mat_list.append(
                np.random.randn(self.neurons_in_layers[i], self.neurons_in_layers[i + 1]) * np.sqrt(
                    2 / self.neurons_in_layers[i]))

        # final step
        self.weights_mat_list.append(
            np.random.randn(self.neurons_in_layers[-1], self.output_size) * np.sqrt(2 / self.neurons_in_layers[-1]))
    ...
    def save(self):
        ...
        for i, (weight, bias) in enumerate(zip(self.weights_mat_list, self.bias_list)):
            np.save(f"Models/{self.model_name}/weight_{i}.npy", weight)
            np.save(f"Models/{self.model_name}/bias_{i}.npy", bias)
    ...
    def load(self):
        ...
        for i in range(len(self.neurons_in_layers) + 1):
            self.weights_mat_list.append(np.load(f"Models/{self.model_name}/weight_{i}.npy"))
            self.bias_list.append(np.load(f"Models/{self.model_name}/bias_{i}.npy"))
    ...
    def predict(self, data):
        ...
        decision = np.argmax(final_score)
        print(final_score)
    ...
    def back_propagation(self, input_data: np.ndarray, expected_output: np.ndarray, actual_output: np.ndarray,
                         learning_rate: float):
        """
