...
class NeuralNetwork:
    ...
    def generate_weights(self, random=True):
        """
        ...
        if random:
            ...
            self.weights_mat_list.append(
                np.ones((self.input_size, self.neurons_in_layers[0])))

            # hidden layers
            for i in range(self.hidden_layer_num - 1):
                self.weights_mat_list.append(
                    np.ones((self.neurons_in_layers[i], self.neurons_in_layers[i + 1])))

            # final step
            self.weights_mat_list.append(
                np.ones((self.neurons_in_layers[-1], self.output_size)))
    ...
    def train(self, input_dataset:np.ndarray, expected_output_dataset:np.ndarray, loop:int):
        ...
        for i in range(loop):
            for j in range(input_dataset.shape[0]):
                ...
                self.back_propagation(input_dataset[j], expected_output_dataset[j], final, 0.0001)
            os.system('clear')
            print(f"Loop: {i}")
    ...
    def back_propagation(self, input_data:np.ndarray, expected_output:np.ndarray, actual_output:np.ndarray, learning_rate:float):
        ...
        bet_outputs_deltas = []
        # final layer
        output_delta = loss(actual_output, expected_output, derive=True)
